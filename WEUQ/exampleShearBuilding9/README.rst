...
Events Using a Basic CFD
^^^^^^^^^^^^^^^^^^^^^^^^

We now revisit the **EVT** panel of WE-UQ to explore the **CFD - Basic** option for defining an event.
...
For this example you will need two additional files: `recorder.tcl <https://github.com/NHERI-SimCenter/WE-UQ/blob/master/examples/9Story/recorder.tcl>`_ and `postprocess.tcl <https://github.com/NHERI-SimCenter/WE-UQ/blob/master/examples/9Story/postprocess.tcl>`_. 
